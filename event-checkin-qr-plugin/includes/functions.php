...
function generar_qr_pdf_personalizado($request, $action_handler) {
    ...
    try {
        ...
        // 2️⃣ Buscar el post del evento por título
        $evento_nombre = isset($request['eventos_2025'][0]) ? sanitize_text_field($request['eventos_2025'][0]) : '';
        $post = get_page_by_title($evento_nombre, OBJECT, 'evento'); // Reemplaza 'evento' por tu CPT si es distinto
        ...
        // 6️⃣ Obtener imagen del evento desde custom field dinámicamente
        if ($post_id) {
            $attachment_id = get_post_meta($post_id, 'imagen_evento', true); // ID dinámico
            if ($attachment_id) {
                ...
                if ($imagen_evento_url) {
                    ...
                    error_log("⚠️ wp_get_attachment_url no devolvió URL para attachment ID: " . $attachment_id);
                }
            } else {
